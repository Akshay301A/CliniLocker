  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    if (emailTrim) {
      ...
      if (emailResult.error) {
        ...
        toast.success(t("Invite email prepared for {{email}}", { email: emailTrim }));
      }
    } else {
        ...
        {showForm && (
          <form onSubmit={handleAdd} className="rounded-xl border border-border bg-card p-6 shadow-card space-y-4">
            ...
            <div className="flex gap-2">
              <Button type="submit" disabled={submitting}>{submitting ? t("Creating invite…") : t("Send invite")}</Button>
